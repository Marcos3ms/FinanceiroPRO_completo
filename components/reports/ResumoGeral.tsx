...
export default function ResumoGeral({ accounts }: { accounts: ResumoConta[] }) {
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3 print:grid-cols-3">
        {accounts.map((acc) => {
          ...
          return (
            <article
              key={acc.accountId}
              className="border border-border bg-bg-card print:break-inside-avoid"
            >
              {/* Cabeçalho: nome da conta | TOTAL */}
              <div className="flex items-baseline justify-between gap-3 border-b border-border bg-bg-elevated px-3 py-2">
                <span className="text-[0.8rem] font-semibold uppercase tracking-wider text-fg-primary">
                  {acc.accountName}
                </span>
              ...
              <div className="px-3 py-2 text-[0.85rem]">
                ...
                {acc.despesas.length === 0 ? (
                  ...
                  acc.despesas.map((d, i) => (
                    <div
                      key={d.categoria}
                      className={`${ROW} border-b border-border py-1`}
                    >
